...
3. **Неделя 3 закрыта 19.08** (3.1–3.5 ✅). Обязательного кода нет. Дальше — опциональная неделя 4 или устная база (`==`/`is`, типы, JWT). ⚠️ претензия к 3.5 (опять моки vs дыры в базе) — `STUDENT_FEEDBACK_for_sonnet.md`.
...
| **3.5**  | Тестирование retry-хелпера через мок | ✅ сдано 19.08 — 3 теста свои; ⚠️ «опять моки / база важнее» в фидбеке |
